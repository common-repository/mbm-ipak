...
class MBM_Ipak_Models_List extends WP_List_Table
{
    ...
    function get_sql($per_page = 5, $page_number = 1, $all_data = false)
    {
        ...
        if (isset($_GET["export_model"])) {
            $data = mbm_ipak\tools::get_sql($per_page, $page_number, true, $this->model_table_name, $this->data_model["fields"], $this->where);
            //  ob_clean();
            // wp_redirect();
            $url=mbm_ipak\tools::get_full_url();
            $url=str_replace('export_model','exportt',$url);
